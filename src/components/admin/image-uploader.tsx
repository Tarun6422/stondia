...
export function ImageUploader({
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const finalFile = uploadFileBlob instanceof File
        ? uploadFileBlob
  ...
  if (value && state === "idle") {
    ...
    return (
      <div className="space-y-2">
        {label && <p className="text-sm font-medium text-foreground">{label}{required && <span className="ml-1 text-destructive">*</span>}</p>}
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-2 text-center">
          ...
          {state === "uploading" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-xs space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{folder}/{fileName}</p>
                </div>
              </div>
              {/* Progress bar */}
...
export function ImageUploaderMultiple({
  ...
  return (
    <div className="space-y-3">
      {label && <p className="text-sm font-medium text-foreground">{label} ({values.length}/{maxFiles})</p>}
      ...
      {values.length < maxFiles && (
        <ImageUploader
          ...
          onClear={() => {}}
          folder={folder}
          description={`JPG, PNG, WebP or PDF up to 10 MB (${maxFiles - values.length} slots left)`}
        />
      )}
      ...
      {values.length > 0 && (
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-5 md:grid-cols-6">
          {values.map((url, i) => (
            <div key={i} className="group relative aspect-square overflow-hidden rounded-lg border border-border/60 bg-card">
              {url.endsWith(".pdf") ? (
